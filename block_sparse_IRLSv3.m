%BLOCK - SPARSE CODE
function [cs,ca,obj,err_cs,err_ca,ws,wa] = block_sparse_IRLSv3(x,Ds,Da,classes_no,attacks_no,blck_size,maxIter,lambda1,lambda2,lambda_reg,alg)
...
err_cs  = ones(maxIter,1);
err_ca  = ones(maxIter,1);
     ...
     elseif alg == 4 %concatenated cs_i and ca_ijs + regularization of ca_i_j_s: second approach
          ...
          for i=1:classes_no
            for j=1:attacks_no
             ...
             del_s(i) = max(sqrt(norm(cs((m/classes_no)*(i-1)+1: i*(m/classes_no)))^2 + sum(norm_ca(i,:))),epsilon);
             ws((m/classes_no)*(i-1)+1: i*(m/classes_no)) = lambda1./del_s(i);
             
            for j=1:attacks_no
              del_1 = sqrt(norm(cs((m/classes_no)*(i-1)+1: i*(m/classes_no)))^2 + sum(norm_ca(i,:)) );
              del_2 = norm_ca(i,j);
              del_a = max(del_1*del_2,epsilon);
             
              wa(  (j-1)*blk_1_size*classes_no + (i-1)*blck_size(j) + 1 : (j-1)*blk_1_size*classes_no + i*blck_size(j)) =  (lambda2*lambda1)/del_a +lambda_reg ;
            end
         end
     end
