...
class BlockSparseActiveSetSolver(object):
    ...
    def _elastic_net_solver(self, x, Ds, Da, sig_active, att_active):
        Ds_blocks = [self.sig_bi.get_block(Ds, i) for i in sig_active]
        Da_blocks = [self.hier_bi.get_block(Da, (i, j)) for i in sig_active for j in att_active]

        set_trace()
        Ds_sub = np.concatenate(Ds_blocks)
        Da_blocks = np.concatenate(Da_blocks)
