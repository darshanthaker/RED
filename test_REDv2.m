...
flag =3
maxIter = 100;
lambda1 = 0.04%0.00026;% 
lambda2 = 0.03%0.01e0%0.00016;
...
subplot(323);imagesc(ca_est_3);colorbar;caption = sprintf('Estimated ca -3, error = %.3f', norm(ca-ca_est_3)/norm(ca));title(caption);subplot(324);imagesc(ca_est_3);colorbar;caption = sprintf('Estimated ca -3, error = %.3f', norm(ca-ca_est_3)/norm(ca));title(caption);
...
subplot(323);imagesc(cs_est_3);colorbar;caption = sprintf('Estimated cs -3, error = %d', norm(cs-cs_est_3)/norm(cs));title(caption);subplot(324);imagesc(cs_est_3);colorbar;caption = sprintf('Estimated cs -3, error = %d', norm(cs-cs_est_3)/norm(cs));title(caption);subplot(325);imagesc(cs_est_no_reg);
...
norm_ca_true = zeros(k,q);
 for i=1:k
      ...
      norm_cs_4(i) = norm(cs_est_no_reg(i,:),2);
 ...
 bl = k*q;
  figure(3);  stem([norm_ca_1(:),norm_ca_2(:),norm_ca_3(:),norm_ca_true(:)],'filled')
  legend('energy 1','energy 2','energy 3','True');
  figure(4); stem([norm_cs_1(:),norm_cs_2(:),norm_cs_3(:),norm_cs_true(:)],'filled');
legend('energy 1','energy 2','energy 3','True');
